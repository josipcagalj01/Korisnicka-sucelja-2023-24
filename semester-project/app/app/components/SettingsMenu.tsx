			...
			<ul className='accountSettingsMenu'>
				{menu.actions.map((action => (
					<li key={action.text}>
						...
						<BorderedLink href={action.basePath + (!action.basePathOnly ? ('/' + action.text.toLowerCase().replaceAll(' ', '-').replaceAll('č', 'c').replaceAll('š', 's')): '') + (action.query ? action.query : '')}>{action.text}</BorderedLink>
					</li>
				)))}
			</ul>
		</div>
