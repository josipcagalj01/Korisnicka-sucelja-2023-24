	...
	try {
		const jsonResponse = await handleUpload({
			...
			onBeforeGenerateToken: async (
				...
				if (isNaN(id)) throw new Error('Invalid form id format provided')
				else {
					const form = await db.form.findUnique({ where: { id: id } })
					if (!form) throw new Error('Form whose id is ... does not exist')
					else if (user?.department_id !== form.department_id && (user?.role_id !== 1 && user?.role_id !== 3)) throw new Error('Access denied')
				}
